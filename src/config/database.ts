...
export function runMigrations(): void {
  ...
  // Migration: add time_limit_seconds column if missing
  const tableExists = db.prepare("SELECT name FROM sqlite_master WHERE name='exercises'").get();
  if (tableExists) {
    const cols = db.prepare("PRAGMA table_info('exercises')").all() as { name: string }[];
    const hasTimeLimit = cols.some((c) => c.name === 'time_limit_seconds');
    if (!hasTimeLimit) {
