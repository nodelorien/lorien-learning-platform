...
function NewUserContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await api.post('/users', { name, company, password, role });
  ...
  return (
    <Container maxWidth="sm" sx={{ py: 4 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
        ...
        <Typography variant="h5">{t('admin.new', lang)} Usuario</Typography>
      </Box>
      ...
      <Paper sx={{ p: { xs: 2, sm: 4 }, borderRadius: 3 }}>
        <form onSubmit={handleSubmit}>
          ...
          <TextField fullWidth label="Contraseña" type="password" value={password} onChange={(e) => setPassword(e.target.value)} sx={{ mb: 2 }} required />
...
export default function NewUserPage() {
  return <AdminRoute><NewUserContent /></AdminRoute>;
}
